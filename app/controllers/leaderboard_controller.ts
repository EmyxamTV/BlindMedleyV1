...
export default class LeaderboardController {
  async index({ inertia, request, auth }: HttpContext) {
    ...
    // Leaderboard amis
    const allFriendships = await Friendship.query()
      .where((q) => {
    ...
    const relationshipByUserId = new Map<number, { id: number; status: string; incoming: boolean }>()
    for (const friendship of allFriendships) {
      relationshipByUserId.set(
        ...
        { id: friendship.id, status: friendship.status, incoming: friendship.addresseeId === auth.user!.id }
      )
    }
