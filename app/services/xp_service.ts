...
export class XpService {
  ...
  async awardXp(userId: number, amount: number, player: GamePlayer): Promise<void> {
    ...
    const newAvgScore = Math.round((profile.avgScore * profile.totalGames + player.score) / (profile.totalGames + 1))
  ...
  private async checkAchievements(userId: number, profile: Profile, player: GamePlayer): Promise<void> {
