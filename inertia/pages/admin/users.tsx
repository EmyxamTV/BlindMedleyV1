    ...
    <div className="admin-page">
      ...
      <div className="table-wrap">
        <table className="data-table">
          ...
          <tbody>
            {users.length === 0 ? (
              <tr><td colSpan={5} style={{ textAlign: 'center', color: 'var(--text-3)', padding: '2rem' }}>Aucun utilisateur trouvé</td></tr>
            ) : users.map((u) => (
              <tr key={u.id} className={u.status === 'banned' ? 'row-banned' : ''}>
                ...
                <td><span className={`status-badge status-${u.status}`}>{STATUS_LABELS[u.status] ?? u.status}</span></td>
      ...
      <div className="pager">
        {meta.currentPage > 1 && (
          <button
            ...
            onClick={() => router.get('/admin/users', { search, status: statusFilter, page: meta.currentPage - 1 })}
          >
            ← Précédent
          </button>
        )}
