          ...
          <table className="data-table">
            ...
            <tbody>
              {recentGames.length === 0 ? (
                <tr><td colSpan={5} style={{ textAlign: 'center', color: 'var(--text-3)', padding: '2rem' }}>Aucune partie récente</td></tr>
              ) : recentGames.map((g) => (
                <tr key={g.id}>
                  <td><span className="admin-id">#{g.id}</span></td>
                  <td><span className={`mode-badge mode-${g.mode}`}>{g.mode}</span></td>
                  <td>{g.playlistName}</td>
                  <td><span className={`status-badge status-${g.status}`}>{statusLabel(g.status)}</span></td>
